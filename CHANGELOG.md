...
| 类别 | 文件 | 内容 |
|------|------|------|
| 🖼️ 背景图 8 张 | `web/public/bg_*.png/webp` | **全面抄袭** ming-salvage-sim 视觉方案：state/court/chat/edict/harem/loading/node + chat.webp，合计 32.4 MB |
| 🎨 视觉 CSS | `web/src/styles/app.css` | 追加 100 行古风背景样式（v0.9.4 theme.py 17 色升级） |
...
| 元素 | 背景 | 用途 |
|------|------|------|
| `.modal-bg-state` | `bg_state.png`（朝会大殿） | 总览/月末奏章 |
| `.modal-bg-chat` | 半透明白 80% | 召对/大臣对话 |
| `.modal-bg-edict` | 半透明白 80% | 诏书 |
| `.court-drawer` | `bg_chat.webp` | 朝堂抽屉 |
| `.harem-drawer` | `bg_harem.png` | 后宫抽屉 |

### 🐛 顺手发现

| 现象 | 后续 |
|------|------|
| 明末背景是**紫禁城/明制建筑** | 主公明示"满足东汉末年+全面抄袭明末"，弟子轻量复用图，未单独绘东汉图（**v1.19.0+ 升级版时可换**） |
| `score_extractor.md` 仍是明朝版（7词26次漏网） | **不在本任务范围**（v1.18.0 = 古风背景），弟子上次记错 |
| v0.9.4 早做过古风 17 色 | v1.18.0 升级版，**保留原 17 色 + 加明末弹窗背景层** |
