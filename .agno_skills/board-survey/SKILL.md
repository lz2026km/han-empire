...
- 诏书涉及某州/某军，需要查该地区或军队的具体数值。
- 判断候选事件是否触发前，需要核实盘面压力。

使用方式（严格按顺序）：
1. `view_state()` — 国势四量表 + 派系 + 阶级 + 诸侯势力总览
2. `check_treasury()` — 钱粮收支详情
3. `list_regions()` — 各州郡危情概览；诏书涉及哪郡就 `inspect_region(region_name)` 单独查
4. `list_armies()` — 军队欠饷/补给警讯；涉及哪军就 `inspect_army(army_name)` 单独查
5. `list_issues()` / `inspect_issue(issue_id)` — 在办事项及进度详情
6. `list_external_powers()` — 董卓/袁绍/曹操/吕布/袁术当前态势
