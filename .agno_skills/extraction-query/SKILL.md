...
description: 档房书办逐章扫邸报、写数值 delta 前，查地区/军队/诸侯势力/朝臣/局势/派系阶级当前值时使用。
---

你处理的是"查盘面当前值，核边界，算 delta"。写任何数值前必须先查，不凭印象。

适用场景：
- 写 region_delta 前，需要知道某州当前 public_support/unrest/corruption 等值。
- 写 army_delta 前，需要知道某军当前 morale/arrears/supply 等值。
- 写 external_power_updates 前，需要知道董卓/袁绍等当前 leverage/military_strength 等值。
...
- 诸侯势力当前值：`get_external_power(power_id)` — 如 dongzhuo/yuan_shao/cao_cao/lv_bu/yuan_shu
