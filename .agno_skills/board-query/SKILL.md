---
name: board-query
description: 推演官/档房书办查盘面当前数值时使用——涵盖国势、钱粮、州郡、军队、诸侯势力、朝臣、局势、派系。写任何数值前必须先查。
---
...
| 工具 | 用途 |
|------|------|
| `view_state()` | 国势四量表（汉室库/内库/民心/威权）+ 派系/阶级/诸侯势力总览 |
| `check_treasury()` | 钱粮收支详情：月入月出、欠饷合计、赤字 |
| `list_regions()` | 各州郡概览（公支/动荡/腐败警讯） |
| `inspect_region(region)` | 指定州郡完整数值；region 传郡名（"河南"）或 id（"henan"）均可 |
| `list_armies()` | 军队欠饷/补给警讯一览 |
| `inspect_army(army)` | 指定军队完整数值；army 传军名（"西凉军"）或 id 均可 |
| `list_external_powers()` | 董卓/袁绍/曹操/吕布/袁术当前态势总览 |
| `inspect_external_power(power)` | 指定诸侯势力完整数值；power 传名称或 id 均可 |
...
- **州郡**：写 region_delta 前先 `inspect_region`，核边界（0~100），超出截到边界。
- **军队**：写 army_delta 前先 `inspect_army`，核 morale/arrears/supply。
- **诸侯势力**：写 external_power_updates 前先 `inspect_external_power`。
